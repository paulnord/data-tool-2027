...
**Export** in the top toolbar downloads **SVG**, **PDF**, or **PNG** with the displayed colors, markers and optional model guides. For collision and multi-interval analyses, the visible graphs are stacked in one figure. SVG keeps editable vector artwork and text. PDF contains a cropped vector figure with embedded fonts; it is the simplest option for inserting a sharp graph into an Overleaf paper. PNG is rendered at up to three times the displayed graph dimensions for applications that require an image. Figure exports contain the graphs and axes; use **Print** for the complete report. A current single fit can also export a ZIP analysis bundle containing `data.csv`, `analysis.json`, Python/SciPy and C++/ROOT programs, and run instructions. The programs read the CSV instead of embedding observations and accept another compatible data file. They require their respective external environments and are not executed by the browser.
