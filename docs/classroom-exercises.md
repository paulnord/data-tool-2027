...
Use the **Examples** menu to open the built-in CSV datasets directly, or choose **Data… → Load file…** to browse ordinary files. The desktop file chooser initially opens the bundled Examples directory, which also contains prepared `.trksess` analyses. The same files live in `examples/data/` in the checkout. CSVs can also be shared, opened through **Data…**, pasted, or used by another analysis program. They contain column headings, explicit units where supplied, and observations. Generated examples include a comment identifying their synthetic origin. They do not contain hidden fit settings or executable generators.
