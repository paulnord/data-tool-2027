...
The same residual plots, confidence bands, reports and print preview are used. Reports include the exact equation, independent variable, starting/fixed values and units. A custom `.trksess` stores these as session v7; it stores no fitted-result cache. See [integration compatibility](integration.md).
