...
workspace, equation families, fit guides, number display, and SciPy/ROOT analysis
...
- New model identifiers use the documented session-v4 migration. Existing v1–v3
  schemas are unchanged; requests and acknowledgments remain v1. Native envelope
  validation accepts the new session version.
...
- `npm test`: 240 tests in 31 files passed.
- `npm run build` and `npm run build:web`: passed.
- `npm run test:e2e`: all 156 browser tests passed on an isolated local server.
- `npm run test:web`: all 3 packaged-web tests passed.
- `npm run test:desktop`: all 5 native tests passed.
- `npm run test:code-exports`: all 22 executable cases passed with SciPy 1.18.1
  ...
  families, fixed parameters, guides, comments, quoted labels, zero scatter, and
