...
Custom equations are emitted from the validated syntax tree, with real-valued numeric literals in C++ and safe parameter aliases in Python. The application does not execute either external environment. The detailed v1 exporter remains developer reference material in `scripts/reference/fullCodeExport.ts`, not a selectable output mode. Browser and native builds create the archive locally; native saving remains bounded and atomic. See [the workflow and statistical conventions](model-comparison-and-code-export.md).
