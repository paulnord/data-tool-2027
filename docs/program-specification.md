...
The source table retains its textual cells, headings, column mappings, unit declarations, and row identities separately from the selected numerical X/Y snapshot. Extra columns and textual information remain available when a user changes assignments. An unmistakably textual leading column is retained as row labels rather than offered as a numerical analysis column. Display rounding MUST NOT replace stored numerical values or source cell text. A CSV export may update headings to express current units; a session retains the richer analysis information.

When present, source row labels appear in observation tables, graph tooltips, copied fit reports and reproducibility metadata. Otherwise those surfaces use human-readable row numbers: the 1-based position in the complete observation list, excluding headings. Excluded or missing observations leave gaps in fitted-result tables instead of renumbering the fit subset. Internal UUIDs and imported row IDs remain unchanged for selection, undo, and session compatibility; they are not displayed as row labels.
...
3. **Presentation preferences:** interface scale, plot colors/markers, visible axis ranges/scales, confidence-band/error-bar visibility, residual-plot visibility, report-section choices, print layout, physical figure-export settings, and whether advanced choices are shown on this device.

Current presentation preferences are held in the running window and are not stored in `.trksess`. The Advanced Features preference is stored locally for the current browser profile or desktop webview; it is neither session state nor a claim about the capabilities used by a saved analysis. Changing appearance, axis view, or feature visibility does not change observations or trigger an optimization. An old result cannot appear current after its scientific inputs change. Sessions save inputs and settings, not a trusted cached fit result.

## 3. Main workflows

### 3.1 Start, load, inspect, apply

The first file-oriented action in the main toolbar is **Data…**. It opens a data workspace that supports **Load file…**, pasted spreadsheet data, a new table, editing, row-label recognition, and **Export CSV…**. Supported file types are `.csv`, `.tsv`, `.txt`, versioned `.json` and `.trksess`, and read-only Tracker `.trk`/`.trz` inputs. Ordinary-fit X, Y and optional Y-uncertainty assignments live with the model in the Analysis panel; collision and multi-interval analyses retain their specialized assignment controls.

Loading is a staged operation. A user first reviews the source table, header rows, suggested numerical roles, units, missing values, and errors, then chooses **Use these data**. **Cancel** preserves the active analysis. Once loaded, the Analysis panel controls ordinary-fit X, Y and optional Y-uncertainty assignments. A malformed or unsupported file MUST NOT replace valid work. Replacing an edited table draft offers an explicit keep/discard choice; replacing unsaved analysis work likewise offers **Keep working** or **Discard changes**.
...
**Examples:** the Data workspace has an Examples selector for bundled ordinary files, including published-data `.trksess` sessions. Synthetic data and Published studies are separate visible categories with item counts and an explicit scrolling cue. “Published” describes source provenance, not a claim that every study reproduces its publication. Published examples are labeled Reproduced, Close, Comparison, Not reproduced or Unsupported according to the documented literature comparison. Each choice uses the ordinary import/review path, and startup stays empty. The user explicitly requested this selector and its September 2026 reorganization. That task-specific authorization takes precedence over the general no-menu wording still present in `AGENTS.md`; this specification records the implemented behavior without rewriting the contribution policy.
...
The single-fit catalog includes linear-basis and nonlinear built-ins plus custom equations. Preserve model identifiers and physical parameter meaning; presentation labels may improve. Parameter order, defaults, domain validation, and unit derivation are defined in [schema.ts](../src/core/fit/schema.ts), [solve.ts](../src/core/fit/solve.ts), [seriesModels.ts](../src/core/fit/seriesModels.ts), and [nonlinearModels.ts](../src/core/fit/nonlinearModels.ts).

| Family / identifier     | Equation or role                                      | Important distinction                                                             |
| ----------------------- | ----------------------------------------------------- | --------------------------------------------------------------------------------- |
| `line`                  | `b + m*x`                                             | Intercept and slope.                                                              |
| Polynomial identifiers  | One family through degree 2–10                        | Power, fixed-center Taylor, or fixed-center/fixed-scale Chebyshev representation. |
...
| `fourier`               | Fixed-period series with 1–5 sine/cosine harmonics    | Period and origin are fixed metadata; amplitudes use linear QR.                   |
...
Polynomial degree remains one selector from 2 through 10. Advanced Features exposes three representations of that same degree-specific curve space:

\[
\sum_{i=0}^{N}c_i x^i,\qquad
\sum_{i=0}^{N}c_i(x-x_0)^i/i!,\qquad
\sum_{i=0}^{N}c_iT_i((x-x_0)/s).
\]

The Taylor center and Chebyshev center/positive scale are fixed, persisted model metadata. They MUST NOT be silently recomputed after row exclusion, interval selection, or session restoration. At full rank, with all coefficients free or fixed constraints transformed equivalently, the three representations have equal predictions, residual objective, degrees of freedom, and comparison parameter count, while their coefficients, coefficient units, covariance, and conditioning differ. Fixing the same numeric coefficient in different representations is not an equivalent constraint. Powers of x is the canonical omitted-basis representation in v7, preserving the prior settings shape. An explicit power marker remains valid input but is removed on the next save; Taylor, Chebyshev, and Fourier sessions require a build that knows their advanced metadata.

The advanced fixed-period Fourier model is

\[
y=b+\sum_{k=1}^{H}\left[s_k\sin(2\pi k(x-x_0)/T)+c_k\cos(2\pi k(x-x_0)/T)\right],\quad 1\le H\le5.
\]

The period \(T>0\) and origin \(x_0\) are fixed, persisted metadata; they are not counted as fitted parameters. Harmonic coefficients and the background have Y units. This model MUST NOT silently become a period search or automatic harmonic-selection procedure.
...
Models linear in free parameters use column-scaled, pivoted, twice-reorthogonalized QR; subtract fixed contributions before solving. This includes all three polynomial representations and fixed-period Fourier series. Do not explicitly invert normal equations. The current normalized rank threshold is `1e-12`. Report numerical rank and the defined conditioning diagnostics, and transform parameters/covariance back to the declared coefficient representation. The current `diagonalRatio` is a ratio of scaled QR diagonals, not a matrix condition number; a ratio above `1e8` produces a warning, not a separate hard failure cutoff. Rank loss cannot produce a supposedly unique fitted parameter vector with ordinary uncertainty.
...
**Display** holds interface size and graph appearance together. **Settings** separates global graph visibility from single-fit copy-report sections. **Show residual plots** defaults on and **Show fit guides when available** defaults off; the full-page print preference is available only in Print preview. **Show advanced models and analysis tools** defaults off and is a device-local discoverability preference. When enabled it reveals Custom equation, supplied-exponent Power law, Reciprocal, Logarithmic, Logistic sigmoid, adjustable Gaussian shape, Fourier series, alternate polynomial representations, and the model-comparison, multi-interval, and collision workspaces. Turning it off hides those choices for new work but MUST leave an already active advanced model or workspace visible and usable, including after a session is opened. It MUST NOT alter scientific state, dirty a session, participate in undo, or be serialized in a session. Axis-specific choices live beside the relevant graph, not in a second general-purpose settings layer. Menus close on outside click or Escape; keyboard dismissal returns focus to their trigger. Action menus support arrow-key navigation. Modal shortcuts stay within the active dialog; closing Data, Figure size, or Print preview restores a visible invoking control. Help text follows interface scale. Popovers must remain reachable on a small laptop at enlarged interface scale.
...
A current fit exports a SciPy or ROOT ZIP containing one standalone program, `data.csv`, `observations.csv`, `analysis.json`, and `README.md`. SciPy also includes `requirements.txt`. There is one export style. The equation is the first function, followed by small data-loading, model-setup, fitting, reporting and plotting functions driven by a main routine. Starts, fixed values, constraints and uncertainty treatment are explicit in the source.

Observations MUST remain data rather than generated source literals. `data.csv` contains the selected finite X/Y observations and supplied sigma when applicable. `observations.csv` preserves the full-precision original table, row identities, inclusion flags and missing-value reasons. Both preserve source comments. Metadata records provenance, units, assumptions, equation/settings, original coefficients, selected row IDs and display preferences for reference; it is not a runtime input. Analysis bundle format v2 documents the new numeric CSV layout; downloaded v1 bundles remain runnable, and session/request/ack formats are unaffected.

The programs use standard NumPy/SciPy or ROOT APIs to refit and display a basic data/fit plot, saving PNG or PDF respectively. Detailed diagnostics, residual panels, fit guides and figure styling remain available through the app's report and figure exports. Supplied uncertainties retain their absolute scale. Unknown equal scatter uses covariance scaled by SSE/df. Fixed parameters and unavailable inferential uncertainties are labeled explicitly. ROOT checks optimization status and covariance quality before saving a successful plot. Nonlinear covariance conventions, numerical conditioning, starting values and local minima can affect agreement with the application.

Custom equations are emitted from the validated syntax tree, with real-valued numeric literals in C++ and safe parameter aliases in Python. Power, fixed-center Taylor, fixed-center/fixed-scale Chebyshev, and fixed-period Fourier models emit their declared bases in both SciPy and ROOT output; SciPy also receives their analytic design matrices. The fixed metadata is recorded in `analysis.json`. The application does not execute either external environment. The detailed v1 exporter remains developer reference material in `scripts/reference/fullCodeExport.ts`, not a selectable output mode. Browser and native builds create the archive locally; native saving remains bounded and atomic. See [the workflow and statistical conventions](model-comparison-and-code-export.md).

## 7. Multiple fits and intervals

A separate **Analysis tools** selector switches among **Single fit**, model comparison, collision and multi-interval workspaces. It occupies the same first position in the Analysis controls for every workspace, even though the latter three choices are normally hidden until Advanced Features is enabled. The ordinary **Model** selector contains equation families rather than workspace choices. The workspaces use the same scientific core. Session v7 preserves the active workspace setup and observations; results are recalculated explicitly after reopening.

### 7.1 Model comparison

The comparison workspace stages two to six candidates. Each candidate can use the current analysis, choose a built-in model, or load a validated session. All candidates are refitted; loaded results are never trusted as cached fit output because sessions retain requests/settings rather than solver results. Failed loads preserve the existing candidate and the main analysis. Each candidate identifies its source and provides **Use current analysis** for an explicit refresh. The top Copy report, Print, and SVG/PNG/PDF export actions operate on the completed comparison. Printing uses measured pages with a transposed statistics table and optional rotated graph page; graph exports support physical sizing. Supplied ±1σ Y error bars appear on included data observations under the shared visibility preference. The paired graphs use equal inner widths, a shared X scale and bottom label, a compact gap, and a 3:1 data-to-residual height ratio. Residual ranges follow the actual values without an absolute unit-dependent floor.
...
Equal-degree power, Taylor, and Chebyshev polynomials with equivalent free/fixed constraints are reparameterizations of the same candidate space, not distinct model-comparison hypotheses. With full rank they have the same likelihood and fitted-parameter count, so AIC/AICc/BIC cannot prefer one representation; any material discrepancy is a numerical diagnostic to investigate. Using the same numeric fixed coefficient across bases instead compares different constrained models.

### 7.2 Multi-interval fits

Select one X and one Y column, then define up to five named intervals. Ranges start empty. A user selects an interval, drags a graph range, adjusts boundary handles or exact limits, selects an equation, edits its starting/fixed values, and explicitly fits that interval. Boundary handles support keyboard arrows. The shared model catalog applies, including its Advanced Features visibility rule. Existing v7 workspaces with multiple Y series remain valid compatibility inputs and round-trip without dropping their per-series setup, but the interface does not create or expand such workspaces.

Built-in nonlinear starting estimates use the selected Y observations within the selected interval, in the selected X units. Selecting a model before defining a range defers useful estimates until observations are available. Untouched estimates follow range or column changes; manually edited starting values or fixed flags are preserved for that interval until explicit model reselection. These estimates do not trigger optimization or guarantee convergence.
...
Intervals include endpoints and may overlap. Missing observations in the selected series are omitted explicitly. The graph shows the full selected data, intervals, and valid fitted segments, with residuals on shared X. Axes in this draft are linear. Changing one interval's scientific settings invalidates that interval; column or uncertainty changes invalidate all affected results. Changing X also clears ranges. Superseded worker work cannot reappear.

This mode offers residual-estimated scatter or one positive common Y sigma. It does not inherit single-fit row exclusions or per-row uncertainties. Relevant known-false source assumptions still block inference. There are no linked parameters, enforced continuity, automatic event detection, derived charge/force/impulse quantities, or cross-fit covariance, including where intervals reuse the same observations.

Copy output places full interval report blocks side by side for spreadsheets. Printing offers an overview graph/results and an optional residual/diagnostic appendix. Legacy multi-series sessions retain one graph and result block per saved Y series. Source notes remain available. See [multi-interval.md](multi-interval.md) for scope and tests.
...
| Polynomial representations and fixed-period Fourier series                                         | Same-degree power/Taylor/Chebyshev predictions and objective agree; fixed metadata round-trips; Fourier coefficients, rank behavior, reports, and SciPy/ROOT output match declared bases. `seriesModels.test.ts`, `codeExport.test.ts`. |
| Advanced Features visibility                                                                       | Default off; opt-in persists only on the device; common choices remain; active advanced sessions stay operable when off; toggling does not change or dirty scientific state. `advanced-features.spec.ts`.                               |
...
Possible future methods—Bessel-function models, component-decomposition plots, general parameter bounds, robust regression, X-error fitting, correlated-error solvers, linked parameters, full uncertainty budgets, comparison of dependent/correlated fits, or specialized nested-model tests—are separate scientific projects. Bessel fitting in particular needs an explicit order/argument/normalization contract and independent numerical references; component plots need consistent semantics across all workspaces and must distinguish basis terms from independently meaningful physics. Each addition needs explicit equations, supported assumptions, failure semantics, independent references, and migration decisions. A rebuilt interface must not imply these methods exist merely because it can display their names.
