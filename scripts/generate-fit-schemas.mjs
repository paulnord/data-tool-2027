...
const bundled = await build({
  stdin: {
    contents:
      'export * from "./src/core/fit/schema.ts"; export * from "./tests/support/synthetic.ts";',
...
const demos = [
  {
    model: "cubic",
    formula: "y=1-2x+0.5x^2+x^3",
    x: (i) => -2 + (4 * i) / 80,
    y: (x) => 1 - 2 * x + 0.5 * x * x + x ** 3,
  },
  {
    model: "quartic",
    formula: "y=1-2x^2+0.5x^4",
    x: (i) => -2 + (4 * i) / 80,
    y: (x) => 1 - 2 * x * x + 0.5 * x ** 4,
  },
  {
    model: "logarithmic",
    formula: "y=1.5+2*ln(x/1)",
    x: (i) => 0.2 + (7.8 * i) / 80,
    y: (x) => 1.5 + 2 * Math.log(x),
  },
  {
    model: "sine",
    formula: "y=1+2*sin(2*pi*x/3)+0.75*cos(2*pi*x/3); supplied period=3 s",
    x: (i) => (9 * i) / 80,
    y: (x) =>
      1 +
...
for (const [index, demo] of demos.entries()) {
  ...
  request.dataset.xColumn = {
    id: "x",
    label: demo.model === "sine" ? "Time" : "x",
    unit: demo.model === "sine" ? "s" : null,
  };
  ...
  writeFileSync(
    `examples/fit/${demo.model}-demo.trksess`,
    JSON.stringify(session, null, 2) + "\n",
  );
}

const freeSine = JSON.parse(
  (await import("node:fs")).readFileSync(
    "examples/fit/sine-demo.trksess",
    "utf8",
  ),
);
...
freeSine.request.dataset.label =
  "Sine with unknown period · synthetic experiment";
...
writeFileSync(
  "examples/fit/sine-fit-period-demo.trksess",
  JSON.stringify(sessionSchema.parse(freeSine), null, 2) + "\n",
);
for (const [index, model, shape, formula, fn] of [
  ...
  request.dataset.yColumn = { id: "y", label: "Signal", unit: "m" };
  request.requestId =
    "e7c00000-0000-4000-8000-" + String(300 + 2 * index).padStart(12, "0");
  request.snapshotId =
    "e7c00000-0000-4000-8000-" + String(301 + 2 * index).padStart(12, "0");
  request.source.context =
    formula + "; Gaussian sigma=0.05 m; seed=" + (4000 + index);
  request.uncertainty.sigmaY = 0.05;
  request.dataset.rows = Array.from({ length: 81 }, (_, i) => {
    const x = 0.25 + (4.75 * i) / 80;
    return {
      id: "row-" + i,
      x,
      y: fn(x) + 0.05 * noise(),
  ...
  writeFileSync(
    "examples/fit/" + model + "-demo.trksess",
    JSON.stringify(
      sessionSchema.parse({
        workspace: { kind: "single-fit" },
        view: { showResiduals: true, showGuides: false, showErrorBars: true },
...
barRequest.source.context =
  "y=1+2x; independent Gaussian sigma_i=0.04+0.18x m; seed=50272027";
const sigmaByRow = {};
barRequest.dataset.rows = Array.from({ length: 31 }, (_, i) => {
  ...
  return {
    id,
    x,
    y: 1 + 2 * x + sigma * barNoise(),
    included: true,
    missingReason: null,
  };
});
barRequest.uncertainty = {
  kind: "supplied-per-row",
  errorStructure: "uncorrelated",
  sigmaByRow,
  provenance: {
    kind: "user-asserted",
    description:
      "Known generating Gaussian standard deviations, synthetic data",
  },
};
writeFileSync(
  "examples/fit/error-bars-demo.trksess",
  JSON.stringify(
    sessionSchema.parse({
      workspace: { kind: "single-fit" },
      view: { showResiduals: true, showGuides: false, showErrorBars: true },
...
unequalRequest.source.context +=
  "; fit weights w_i=1/sigma_i^2 (625 to 6.25 m^-2)";
const unequalSession = sessionSchema.parse({
  workspace: { kind: "single-fit" },
  view: { showResiduals: true, showGuides: false, showErrorBars: true },
  format: "tracker-fit-session",
  version: 7,
  engine: "qr-vp-sine-2",
  request: unequalRequest,
  settings: initialSettings("line"),
});
writeFileSync(
  "examples/fit/unequal-weights-demo.trksess",
  JSON.stringify(unequalSession, null, 2) + "\n",
);
...
equalSession.request.uncertainty = {
  kind: "unknown-equal",
  errorStructure: "uncorrelated",
};
// Deliberately withhold inference assertions for this misspecified noise model.
equalSession.request.dataset.assumptions = {
  exactX: "asserted",
  gaussianIndependent: "unknown",
  correctModel: "unknown",
};
equalSession.request.source.context +=
  "; control: true unequal uncertainties withheld from fitter; equal weighting is intentionally misspecified; inference assertions withheld for descriptive comparison";
writeFileSync(
  "examples/fit/equal-weights-comparison.trksess",
  JSON.stringify(sessionSchema.parse(equalSession), null, 2) + "\n",
);
writeFileSync(
  "examples/fit/unequal-weights-data.csv",
  [
    "row_id,x_s,y_m,sigma_y_m,inverse_variance_weight",
    ...unequalRequest.dataset.rows.map((row) =>
      [
        row.id,
        row.x,
        row.y,
        sigmaByRow[row.id],
        1 / sigmaByRow[row.id] ** 2,
      ].join(","),
    ),
  ].join("\n") + "\n",
);
