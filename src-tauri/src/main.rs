...
fn validate_fit_json(data: &str) -> Result<()> {
    ...
    if !((value["version"] == 1 && matches!(value["format"].as_str(),Some("tracker-fit-request"|"tracker-fit-session"))) || ((value["version"] == 2 || value["version"] == 3 || value["version"] == 4) && value["format"] == "tracker-fit-session")) {return Err("Unsupported fit file".into())}
    Ok(())
}
#[tauri::command]
...
mod tests {
    ...
    #[test] fn legacy_envelopes_and_acknowledgments() {
        ...
        assert!(validate_fit_json(r#"{"format":"tracker-fit-session","version":5}"#).is_err());
